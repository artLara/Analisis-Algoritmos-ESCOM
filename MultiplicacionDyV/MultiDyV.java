...
public  class MultiDyV{
    ...
    private static void inicio(){
        ...
        while(seguir){
            ...
            if(s.nextInt()==10){
                ...
                /*recuperarNumerosDigitos();
                generarNumerosBi();*/
                imp("\tMultiplicacion="+quitarDecimal(mulBi(a,b)+""));
            }
    ...
    private static double mulBi(String numA, String numB){
        if(numA.length()==1 || numB.length()==1){
            /*imp("mulA="+numA);
            imp("mulB="+numB);
            imp("MUlti="+(Double.parseDouble(numA)*Double.parseDouble(numB)));*/
            return Double.parseDouble(numA)*Double.parseDouble(numB);
        } else{
            String iA= numA.substring(0,numA.length()/2);
            String dA= numA.substring(numA.length()/2,numA.length());
            ...
            imp("dB="+dB);
            imp("");*/
            return sumBi(quitarDecimal(Math.pow(10,dA.length())*Math.pow(10,dB.length())*mulBi(iA,iB) + ""),
                   quitarDecimal(Math.pow(10,dA.length())*mulBi(iA,dB) + ""),
                   quitarDecimal(Math.pow(10,dB.length())*mulBi(dA,iB) + ""),
                   quitarDecimal(mulBi(dA,dB) + ""));
        }
    }

    private static String quitarDecimal(String ss){
        //imp("ss="+ss);
        StringTokenizer t;
        String aux="";
        String aux2="";
        int count;
        if(ss.contains(".") && ss.contains("E")){
            t = new StringTokenizer(ss,"E");
            aux2=t.nextToken();
            count=Integer.parseInt(t.nextToken());
            //imp("Numero sin exponente="+aux2);
            //imp("Numero de exponente:"+count);
            t = new StringTokenizer(aux2,".");
            aux2=t.nextToken();
            aux=t.nextToken();//digitos despues del punto decimal
            //imp("Numero antes del punto="+aux2);
            //imp("Numero despues del punto="+aux);
            t = new StringTokenizer(aux2,",");
            aux2="";
            while(t.hasMoreTokens()){
                aux2+=t.nextToken();
            }
            //imp("Quitando comas="+aux2);
            aux2+=aux;
            //imp("Agregando decimales="+aux2);
            count-=aux.length();
            //imp("Count="+count);
